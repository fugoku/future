...
import { POPULAR_ARTICLES } from "@/lib/constant";
import { allPosts } from "contentlayer/generated";
import { useRouter } from "next/navigation";
import Highlighter from "react-highlight-words";
import { APP_HOSTNAMES, HOME_DOMAIN } from "@/lib/constant";
    ...
    >
      <Command label="CMDK" loop shouldFilter={false}>
        <Command.Input
          ...
          className="w-full border-none p-4 font-normal placeholder:text-gray-400 focus:outline-none focus:ring-0"
        />
        <Command.List
          ref={commandListRef}
...
const CommandResults = ({
  ...
  const popularArticles = POPULAR_ARTICLES.map(
    (slug) => allPosts.find((post) => post.slug === slug)!,
  );
